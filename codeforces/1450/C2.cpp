...
void solve(){
	...
	int cnt[3][2]; f(i,0,3) f(j,0,2) cnt[i][j]=0; 
	f(i,0,n)
		f(j,0,n) if(s[i][j]=='X') cnt[(i+j)%3][0]++; 
		else if(s[i][j]=='O') cnt[(i+j)%3][1]++;
	int x=-1,o=-1,k=0; f(i,0,3) f(j,0,2) k+=cnt[i][j];
	k/=3;
	f(i,0,3){
		f(j,0,3){
			if(i==j) continue;
			int kl=cnt[i][0]+cnt[j][1];
			if(kl<=k) {
				x=i;
				o=j;
			}	
	...
	f(i,0,n)
		f(j,0,n) if(s[i][j]=='X' && ((i+j)%3)==x) s[i][j]='O';
		else if(s[i][j]=='O' && ((i+j)%3)==o) s[i][j]='X';
	f(i,0,n) cout<<s[i]<<"\n";
}
