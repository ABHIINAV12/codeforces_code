...
void solve(){
	int n; cin>>n;
	array<int,3> lp={0,0,-1};
	st.insert(lp);
	f(i,0,n){
		set<array<int,3>> sp;
		for(auto it : st){
			int dir=it[2]; int x=it[0]; int y=it[1];
			if(dir==-1){
				f(k,0,4){
					int nx=x+dx4[k];
					int ny=y+dy4[k];
					array<int,3> pp={nx,ny,val[k]};
					sp.insert(pp);
				}
			}else if(dir==0 || dir==1){
				f(k,2,4){
...
void B(){
	//1 + ( 3*n*(n + 4) + 2 - (-1)^n*(n*(n + 4) + 2) )/8
	int n; cin>>n;
	int ret=3*n*(n + 4); ret+=2;
	int ano=n*(n + 4) + 2;
	if(n&1) ret+=ano;
	else ret-=ano; 
	ret/=8;
